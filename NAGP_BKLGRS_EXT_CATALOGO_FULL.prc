CREATE OR REPLACE PROCEDURE NAGP_BKLGRS_EXT_CATALOGO_FULL IS
...
BEGIN
    ...
    v_file := UTL_FILE.fopen('BACKLGRS', 'Ext_Bklgrs_Catalogo_'||'Incremental'||'.csv', 'w', 32767);

    -- Pega o nome das colunas para inserir no cabecalho pq tenho preguica
    SELECT LISTAGG(COLUMN_NAME,';') WITHIN GROUP (ORDER BY COLUMN_ID)
      INTO v_Cabecalho
      FROM ALL_TAB_COLUMNS A
     WHERE A.table_name = 'NAGV_BKLGRS_CATALOGO_FULL';
    ...
    -- Executa a query e escreve os resultados
      ...
      FOR bs IN (SELECT *                                           
                    FROM NAGV_BKLGRS_CATALOGO_FULL 
                   WHERE 1=1)
